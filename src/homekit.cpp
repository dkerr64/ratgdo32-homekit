...
void wifiCallbackAll(int count)
{
    ...
    {
        RINFO(TAG, "HomeSpan started in Soft AP mode, not starting ratgdo web services");
        return;
    }
...
void setup_homekit()
{
    ...
    homeSpan.setApSSID(device_name_rfc952);
    ...
    if (userConfig->getSoftAPmode())
    {
        // If in soft AP mode don't set anything up
