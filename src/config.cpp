...
{
    // call fn to set reboot only if required
    userConfig->set(key, value);
    return true;
}
...
{
    // call fn to set system time zone
    userConfig->set(key, value);
    return true;
}

bool helperSoftAPmode(const std::string &key, const std::string &value, configSetting *action)
{
    userConfig->set(key, value);
...
{
    userConfig->set(key, value);
    // thes globals are set to optimize log message handling...
...
userSettings::userSettings()
{
    ...
    settings = {
        ...
        {cfg_softAPmode, {true, false, false, helperSoftAPmode}}, // call fn to trigger HomeSpan's soft AP mode.
