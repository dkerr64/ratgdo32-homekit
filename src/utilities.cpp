...
{
    clockSet = true;
    //RINFO(TAG, "Current time: %s", timeString());
...
void load_all_config_settings()
{
    ...
    if (!userConfig->load())
    {
        RINFO(TAG, "No settings saved, using factory defaults.");
        // Save defaults so next time load will succeed...
        userConfig->save();
    }
    ...
    RINFO(TAG, "RFC952 compliant device hostname: %s", device_name_rfc952);
    RINFO(TAG, "User Configuration...");
    RINFO(TAG, "   deviceName:          %s", userConfig->getDeviceName().c_str());
    RINFO(TAG, "   wifiSettingsChanged: %s", userConfig->getWifiSettingsChanged() ? "true" : "false");
