...
String *ratgdoAuthenticate(HTTPAuthMethod mode, String enteredUsernameOrReq, String extraParams[])
{
    ...
    std::string key = enteredUsernameOrReq.c_str();
    String *pw = new String(nvRam->read(key + "-pw", "password").c_str());
    return pw;
}
...
void handle_checkflash()
{
    // TODO flashCRC = ESP.checkFlashCRC();
...
void handle_everything()
{
    ...
    // to verbose... RINFO(TAG, "Handle everything for %s", uri);
#ifdef AP_MODE
    if ((WiFi.getMode() & WIFI_AP) == WIFI_AP)
    {
        // If we are in Soft Access Point mode
        RINFO(TAG, "WiFi Soft Access Point mode");
        String page = server.uri();
        if (page == "/" || page == "/ap")
...
#define clientCount 0
    ...
    // TODO ADD_BOOL(json, "lockedAP", wifiConf.bssid_set)
...
bool helperCredentials(const std::string &key, const std::string &value, configSetting *action)
{
    ...
    *strchr(newPassword, '"') = (char)0;
    ...
    std::string user = newUsername;
    nvRam->write(user + "-pw", newPassword);
    return true;
}
...
void SSEheartbeat(SSESubscription *s)
{
    ...
    if (s->client.connected())
    {
        ...
        ADD_INT(json, "freeHeap", free_heap);
        ADD_INT(json, "minHeap", min_heap);
        // TODO ADD_INT(json, "minStack", ESP.getFreeContStack());
        ADD_BOOL(json, "checkFlashCRC", flashCRC);
...
void handle_setssid()
{
    ...
    {
        RINFO(TAG, "Requested WiFi SSID: %s (%d)", ssid);
    ...
    {
        RINFO(TAG, "WiFi Failed to connect to SSID: %s", ssid);
        if (connected)
        {
